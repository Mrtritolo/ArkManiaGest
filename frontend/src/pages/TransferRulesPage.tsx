...
const TRANSFER_LEVELS = [
  { value: 0, label: 'Full', color: '#16a34a', bg: 'rgba(22,163,74,0.08)', desc: 'Personaggio + inventario + dino' },
  { value: 1, label: 'Survivor + Inv.', color: '#ca8a04', bg: 'rgba(202,138,4,0.08)', desc: 'Personaggio e inventario' },
  { value: 2, label: 'Solo Survivor', color: '#ea580c', bg: 'rgba(234,88,12,0.08)', desc: 'Solo personaggio' },
  { value: 3, label: 'Bloccato', color: '#dc2626', bg: 'rgba(220,38,38,0.08)', desc: 'Trasferimento negato' },
]
    ...
    <div className="page-container">
      ...
      <div className="page-header">
        <div className="page-header-text">
          <h1 className="page-title"><ArrowRightLeft size={22} /> Transfer Rules</h1>
          <p className="page-subtitle">Regole di trasferimento tra server — {rules.length} configurate</p>
        </div>
      ...
      {showAdd && (
        <div className="card" style={{ padding: '1rem', marginBottom: '1rem', borderLeft: '3px solid var(--accent)' }}>
          <h3 style={{ margin: '0 0 0.75rem', fontSize: '0.9rem', fontWeight: 700 }}>
            <Plus size={14} style={{ verticalAlign: -2 }} /> Nuova Regola
          </h3>
          <form onSubmit={handleAdd}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr 1fr', gap: '0.6rem', alignItems: 'end' }}>
              <div>
                <label style={{ fontSize: '0.68rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--text-secondary)', display: 'block', marginBottom: 3 }}>Sorgente</label>
              ...
              <div>
                <label style={{ fontSize: '0.68rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--text-secondary)', display: 'block', marginBottom: 3 }}>Destinazione</label>
              ...
              <div>
                <label style={{ fontSize: '0.68rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--text-secondary)', display: 'block', marginBottom: 3 }}>Livello</label>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.6rem', alignItems: 'center' }}>
              <input className="input" placeholder="Note opzionali..." value={newRule.notes} onChange={e => setNewRule({ ...newRule, notes: e.target.value })} style={{ flex: 1, fontSize: '0.82rem' }} />
              <button type="submit" className="btn btn-primary" style={{ fontSize: '0.82rem' }}>Crea</button>
              <button type="button" onClick={() => setShowAdd(false)} className="btn btn-ghost" style={{ fontSize: '0.82rem' }}>Annulla</button>
            </div>
          </form>
        </div>
      )}
      ...
      <div className="card" style={{ minHeight: 200 }}>
        {loading ? (
          ...
          <div className="pl-empty" style={{ padding: '3rem' }}>
            ...
            <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Il livello default (dalla config globale) si applica a tutti i trasferimenti</p>
          </div>
        ) : (
          <>
            {/* Header */}
            <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 30px 1.2fr 140px 1fr 80px', padding: '0.5rem 1rem', fontSize: '0.65rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--text-secondary)', background: 'var(--bg-card-muted)', borderBottom: '2px solid var(--border)' }}>
              <span>Sorgente</span><span></span><span>Destinazione</span><span>Livello</span><span>Note</span><span></span>
            </div>
            {/* Rows */}
            {rules.map(rule => {
              ...
              return (
                <div key={rule.id} style={{
                  ...
                  <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>
                    {isEditing ? (
                      <input className="input" value={editNotes} onChange={e => setEditNotes(e.target.value)} placeholder="Note..." style={{ fontSize: '0.78rem', padding: '0.2rem 0.4rem' }} />
                    ) : (
                      rule.notes || '—'
                    )}
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: '0.2rem', justifyContent: 'flex-end' }}>
                    {isEditing ? (
                      <>
                        ...
                        <button onClick={() => setEditingId(null)} title="Annulla" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: 3 }}><X size={15} /></button>
                      </>
                    ) : (
                      <>
                        <button onClick={() => startEdit(rule)} title="Modifica" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: 3 }}><Edit2 size={14} /></button>
                        <button onClick={() => handleDelete(rule.id)} title="Elimina" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--danger)', padding: 3 }}><Trash2 size={14} /></button>
                      </>
                    )}
                  </div>
                </div>
