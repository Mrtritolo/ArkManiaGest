"""
plugin_base.py — Base infrastructure for the ArkShop config editor.

Defines an ``APIRouter`` pre-wired with:

  - Local config CRUD (upload, export)
...
# Settings key used to store the version history list
_VERSIONS_KEY = "arkshop_versions"
# Plugin folder names to match during container scanning (case-insensitive)
_FOLDER_NAMES = ["ArkShop", "arkshop"]
...
def _get_config() -> dict:
    ...
    if not config:
        raise HTTPException(
            status_code=404,
            detail=f"No {_PLUGIN_NAME} configuration saved.",
        )
...
def _folder_matches(name: str) -> bool:
    ...
    return name.lower() in [f.lower() for f in _FOLDER_NAMES]
